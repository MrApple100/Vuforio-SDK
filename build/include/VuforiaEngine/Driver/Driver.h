...
/// A list of supported pixel formats for camera frames
...
/// Enum values to describe state of the camera pose
...
/// Indicate validity (quality) of the pose
...
/// Data structure for storing translation and rotation matrix associated with an anchor
VUFORIA_DRIVER_PACKED_STRUCT(AnchorPose
{
    /// The array contains translation vector values (x, y, z) associated with the anchor
    float translationData[3];

    /// The array contains elements of 3*3 rotation matrix associated with the anchor
    float rotationData[9];
});

/// Data structure for storing information about an anchor
VUFORIA_DRIVER_PACKED_STRUCT(Anchor
{
    /// UUID associated with the anchor
    /**
     * \note A successful call to createAnchor() generates an UUID.
     */
    const char* uuid;

    /// Pose associated with the anchor
    AnchorPose pose;
});

/// Interface to deliver camera frames to %Vuforia
class CameraCallback
{
public:
    /// Callback function invoked when a new camera frame is available
    virtual void VUFORIA_DRIVER_CALLING_CONVENTION onNewCameraFrame(CameraFrame* frame) = 0;
};

/// Interface to deliver poses to %Vuforia
class PoseCallback
{
public:
    /// Callback function invoked when a new pose is available
    virtual void VUFORIA_DRIVER_CALLING_CONVENTION onNewPose(Pose* pose) = 0;
};

/// Interface to deliver anchor updates to %Vuforia
class AnchorCallback
{
public:
    /// Callback function invoked when new anchor information is available
...
public:
    ...
    /**
     *  This function returns the total number of supported camera modes.
...
public:
    ...
    /// Constructs a new instance of an external positional device tracker
    /**
...
extern "C"
{
     ...
     * \param userData User data that the developer provides as the "userData" parameter in the VuDriverConfig Engine configuration data structure
     * \returns New Vuforia Driver instance
     */
