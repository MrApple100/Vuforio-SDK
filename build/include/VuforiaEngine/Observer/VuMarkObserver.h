...
VU_ENUM(VuVuMarkCreationError)
{
    ...
    VU_VUMARK_CREATION_ERROR_TARGET_NOT_FOUND      = 0x5, ///< Specified target not found in database
...
/// \brief Instance Info for VuMark
typedef struct VuVuMarkObservationInstanceInfo
{
    /// \brief The type of data observed VuMark instance ID stores
    VuMarkInstanceIdType dataType;

    /// \brief String buffer, filled with length bytes representing the id data
    /**
    ...
    /// \brief Byte length of the buffer
    int32_t length;
    
    /// \brief Convenience function to get IDs of type NUMERIC
    uint64_t numericValue;
} VuVuMarkObservationInstanceInfo;

/// \brief Info for VuMark observations
 ...
 */
VU_API VuResult VU_API_CALL vuEngineCreateVuMarkObserver(VuEngine* engine, VuObserver** observer, const VuVuMarkConfig* config, VuVuMarkCreationError* errorCode);
...
/// \brief Get the motion hint associated to the target from a VuMark observer
VU_API VuResult VU_API_CALL vuVuMarkObserverGetMotionHint(const VuObserver* observer, VuMotionHint* motionHint);

/// \brief Set the motion hint associated to the target from a VuMark observer
/**
 * \note This operation will reset any tracking operation for a VuMark observer
 ...
 */
VU_API VuResult VU_API_CALL vuVuMarkObserverSetMotionHint(VuObserver* observer, VuMotionHint motionHint);
...
VU_API VuResult VU_API_CALL vuVuMarkObservationGetTemplateInfo(const VuObservation* observation, VuVuMarkObservationTemplateInfo* templateInfo);

/// \brief Get VuMark instance info associated with a VuMark observation
VU_API VuResult VU_API_CALL vuVuMarkObservationGetInstanceInfo(const VuObservation* observation, VuVuMarkObservationInstanceInfo* instanceInfo);
