...
VU_ENUM(VuDevicePoseObservationStatusInfo)
{
    ...
    VU_DEVICE_POSE_OBSERVATION_STATUS_INFO_INSUFFICIENT_FEATURES = 0x7, ///< There are insufficient features available in the scene. This will only
                                                                        ///< appear when the device pose is in Limited status
...
VU_API VuResult VU_API_CALL vuEngineCreateDevicePoseObserver(VuEngine* engine, VuObserver** observer, const VuDevicePoseConfig* config, VuDevicePoseCreationError* errorCode);
...
VU_API VuResult VU_API_CALL vuDevicePoseObservationGetStatusInfo(const VuObservation* observation, VuDevicePoseObservationStatusInfo* statusInfo);
 ...
 *
 * \note Changing the static mode will internally also trigger a device tracking reset and re-initialization. All device pose observations are reset to
 * VU_OBSERVATION_POSE_STATUS_NO_POSE and tracking is lost on any targets tracked by extended tracking.
 * Any anchors created during the session are destroyed.
 */
