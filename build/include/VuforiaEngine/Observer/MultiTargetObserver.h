...
VU_ENUM(VuMultiTargetCreationError)
{
    ...
    VU_MULTI_TARGET_CREATION_ERROR_TARGET_NOT_FOUND      = 0x5, ///< Specified target not found in database
...
VU_ENUM(VuMultiTargetPartsCreationError)
{
    ...
    VU_MULTI_TARGET_PARTS_CREATION_ERROR_DATABASE_LOAD_ERROR   = 0x3, ///< Database file not found or an error occurred when reading data from it
                                                                      ///< (potentially unknown or corrupted file)
    VU_MULTI_TARGET_PARTS_CREATION_ERROR_INVALID_TARGET_NAME   = 0x4, ///< Invalid target name
    VU_MULTI_TARGET_PARTS_CREATION_ERROR_ASSEMBLY_ERROR        = 0x5  ///< An error occurred while trying to set up the parts that form the Multi-Target
};
...
/**
 * \note Note that loading the database may take a significant amount of time, it is therefore recommended that this method is not called on the main/UI thread.
 */
VU_API VuResult VU_API_CALL vuEngineCreateMultiTargetObserver(VuEngine* engine, VuObserver** observer, const VuMultiTargetConfig* config, VuMultiTargetCreationError* errorCode);

/// \brief Create a Multi-Target observer from parts
VU_API VuResult VU_API_CALL vuEngineCreateMultiTargetObserverFromPartsConfig(VuEngine* engine, VuObserver** observer, const VuMultiTargetPartsConfig* config, VuMultiTargetPartsCreationError* errorCode);
...
/// \brief Get the axis-aligned bounding box associated to the target from a Multi-Target observer, relative to the target's frame of reference
...
/// \brief Get the motion hint associated to the target from a Multi Target observer
VU_API VuResult VU_API_CALL vuMultiTargetObserverGetMotionHint(const VuObserver* observer, VuMotionHint* motionHint);

/// \brief Set the motion hint associated to the target from a Multi Target observer
/**
 * \note This operation will reset any tracking operation for a Multi Target observer
 * (recommended to use this function before starting Vuforia engine)
 *
 * \note Only VU_MOTION_HINT_ADAPTIVE and VU_MOTION_HINT_DYNAMIC are allowed
 */
VU_API VuResult VU_API_CALL vuMultiTargetObserverSetMotionHint(VuObserver* observer, VuMotionHint motionHint);
...
VU_API VuResult VU_API_CALL vuMultiTargetObservationGetStatusInfo(const VuObservation* observation, VuMultiTargetObservationStatusInfo* statusInfo);

/// \brief Get target info associated with Multi-Target observation
VU_API VuResult VU_API_CALL vuMultiTargetObservationGetTargetInfo(const VuObservation* observation, VuMultiTargetObservationTargetInfo* targetInfo);
...
VU_API VuResult VU_API_CALL vuMultiTargetPartConfigListGetElement(const VuMultiTargetPartConfigList* list, int32_t element, VuMultiTargetPartConfig* config);

/// \brief Append an element to a Multi-Target part config list
VU_API VuResult VU_API_CALL vuMultiTargetPartConfigListAppendElement(VuMultiTargetPartConfigList* list, const VuMultiTargetPartConfig* config);
...
VU_API VuResult VU_API_CALL vuMultiTargetPartListGetElement(const VuMultiTargetPartList* list, int32_t element, const VuMultiTargetPart** part);
...
/**
 * \note This pose is relative to the Multi-Target to which the part belongs.
 */
