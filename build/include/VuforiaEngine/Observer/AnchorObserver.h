...
/// \brief Target info for anchor
typedef struct VuAnchorObservationInfo
{
    /// \brief Pose offset used with the anchor
...
{
    /// \brief Pose matrix representing pose the anchor should be created at
    VuMatrix44F pose;
...
VU_API VuResult VU_API_CALL vuEngineCreateAnchorObserver(VuEngine* engine, VuObserver** observer, const VuAnchorObserverConfig* config, VuAnchorCreationError* errorCode);
...
VU_API VuResult VU_API_CALL vuAnchorObserverCreateAnchorWithHitTest(VuObserver* observer, const VuAnchorCreationHitTestConfig* config, int32_t* anchorId);
...
/// \brief Get anchor Id associated with an anchor observation
/**
 ...
 */
VU_API VuResult VU_API_CALL vuAnchorObservationGetAnchorId(const VuObservation* observation, int32_t* anchorId);
