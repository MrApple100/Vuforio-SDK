...
VU_ENUM(VuVirtualButtonCreationError)
{
    ...
    VU_VIRTUAL_BUTTON_CREATION_ERROR_INVALID_OBSERVER      = 0x3, ///< Observer is null or of an invalid type, only Image Target observers are supported
 ...
 */
VU_API VuResult VU_API_CALL vuEngineCreateVirtualButtonObserver(VuEngine* engine, VuObserver** observer, const VuVirtualButtonConfig* config, VuVirtualButtonCreationError* errorCode);
...
VU_API VuResult VU_API_CALL vuVirtualButtonObservationGetInfo(const VuObservation* observation, VuVirtualButtonObservationInfo* virtualButtonInfo);
