...
typedef struct VuAreaTargetConfig
{
    ...
    /// \brief Require external positions to be set during the observer lifecycle for 
    /// initial detection and recovery from extended loss of tracking
    /**
 ...
 * \returns VU_SUCCESS on success, VU_FAILED on failure.
 */
VU_API VuResult VU_API_CALL vuAreaTargetObserverSetExternalPositionFrom2D(VuObserver* observer, VuVector2F position, float horizontalAccuracy);
 ...
 * \returns VU_SUCCESS on success, VU_FAILED on failure.
 */
VU_API VuResult VU_API_CALL vuAreaTargetObserverSetExternalPositionFrom3D(VuObserver* observer, VuVector3F position, float horizontalAccuracy, float verticalAccuracy);
