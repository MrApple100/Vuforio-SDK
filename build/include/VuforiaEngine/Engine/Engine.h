 ...
 * Not calling it from the main/UI thread is critical on platforms that require user permissions for accessing hardware
 * such as the camera (e.g. on iOS or Android), as the approval process typically requires the main/UI thread not being
 * blocked.
 *
...
/**
 * \note This will release all resources used by the Engine instance and implicitly stop the Engine instance if it
...
/**
 * \note This may implicitly start any internal resources of observers and controllers that have been activated
 * or started prior to starting the Engine instance.
 *
 * \note Starting the engine instance is a potentially long running operation and should ideally not be called on the
...
/**
 * \note This will also deactivate all observers and stop any running controllers
 * 
