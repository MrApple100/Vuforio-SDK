...
typedef struct VuDeviceCalibrationConfig
{
     ...
     * VU_FALSE = fetch synchronously while blocking the initialization call, causing it to take longer but guaranteeing latest calibration (default value)
     * By default the mode is set to VU_FALSE.
     */
...
VU_API VuResult VU_API_CALL vuEngineConfigSetAddDeviceCalibrationConfig(VuEngineConfigSet* configSet, const VuDeviceCalibrationConfig* config);
