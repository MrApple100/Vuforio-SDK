...
VU_ENUM(VuRecordingCreationError)
{
    ...
    VU_RECORDING_CREATION_ERROR_DATA_FLAGS_NOT_SUPPORTED = 0x1, ///< One or more requested data flags are not supported on the current device
...
VU_ENUM(VuRecordingStartError)
{
    ...
    VU_RECORDING_START_ERROR_INTERNAL                   = 0x1, ///< Starting failed due to an internal error. Please check the device logs for additional information.
    VU_RECORDING_START_ERROR_ANOTHER_RECORDING_RUNNING  = 0x2, ///< Failed to start a recording because another recording is already in progress.
    ...
    VU_RECORDING_START_ERROR_INVALID_STATUS             = 0x4, ///< Cannot start a recording that is not in the initialized state. The user should create a new recording.
    ...
    VU_RECORDING_START_ERROR_DATA_SOURCE                = 0x7, ///< Failed to start recording from some of the selected data sources. This could happen if e.g.
                                                               ///< another application has exclusive access to a source.
    VU_RECORDING_START_ERROR_INSUFFICIENT_FREE_SPACE    = 0x8, ///< Cannot start a recording as there isn't sufficient free space on the device to store it
};
...
VU_ENUM(VuRecordingStatusInfo)
{
    ...
    VU_RECORDING_STATUS_INFO_ERROR_INTERNAL                   = 0x2, ///< Recording was aborted due to an internal error. Please check the device logs for additional information.
    ...
    VU_RECORDING_STATUS_INFO_ERROR_INSUFFICIENT_FREE_SPACE    = 0x5, ///< The recording stopped as there isn't sufficient free space on the device to store it
};
 ...
 */
VU_API VuResult VU_API_CALL vuSessionRecorderControllerGetDefaultRecordingDataFlags(const VuController* controller, VuRecordingDataFlags* dataFlags);
 ...
 * \returns VU_SUCCESS on successful retrieval of the data flags, VU_FAILED on failure
 */
VU_API VuResult VU_API_CALL vuSessionRecorderControllerGetSupportedRecordingDataFlags(const VuController* controller, VuRecordingDataFlags* dataFlags);
 ...
 */
VU_API VuResult VU_API_CALL vuSessionRecorderControllerGetDefaultRecordingFrameRate(const VuController* controller, VuRecordingFrameRate* frameRate);
 ...
 * the specific value being used for recording.
 */
VU_API VuResult VU_API_CALL vuSessionRecorderControllerGetDefaultRecordingImageScale(const VuController* controller, VuRecordingImageScale* scale);
 ...
 */
VU_API VuResult VU_API_CALL vuSessionRecorderControllerCreateRecording(VuController* controller, const VuRecordingConfig* config, VuRecording** recording, VuRecordingCreationError* error);
